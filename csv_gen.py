...
def build_csv():
    ...
    if os.path.exists("C:/Users/aarra/Python/CANSAT/Flight_1076C.csv") == False:
    
        with open('Flight_1076C.csv','w') as file:
            writerCON = csv.writer(file, delimiter = ",")
            writerCON.writerow(headerCON)
            file.close()

        with open('Flight_1076P.csv','w') as file:
...
def append_csv(data):
    ...
    if os.path.exists("C:/Users/aarra/Python/CANSAT/Flight_1076C.csv") == True:
    
        with open('Flight_1076C.csv','a') as file:
            writerCON = csv.writer(file)
            writerCON.writerows(data)
            file.close()
        
        with open('Flight_1076P.csv','a') as file:
            writerPAY = csv.writer(file)
            writerPAY.writerows(data)
            file.close()
    else:
        build_csv()
        ...
        with open('Flight_1076C.csv','a') as file:
            writerCON = csv.writer(file, delimiter = ",")
            writerCON.writerow(data)
            file.close()
        
        with open('Flight_1076P.csv','a') as file:
            writerPAY = csv.writer(file, delimiter = ',')
            writerPAY.writerow(data)
            file.close()
...
def append_pandas(data):
    ...
    if os.path.exists("C:/Users/aarra/Python/CANSAT/Flight_1076C.csv") == True:
        ...
        dataframe.to_csv('C:/Users/aarra/Python/CANSAT/Flight_1076C.csv', mode='a', index=False, header=False)
    else:
        build_csv()
        dataframe = pd.DataFrame(data)
        dataframe.to_csv('C:/Users/aarra/Python/CANSAT/Flight_1076C.csv', mode='a', index=False, header=False )
